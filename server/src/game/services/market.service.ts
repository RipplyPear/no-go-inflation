...
async function getParticipantAndLockedSession(
  ...
  const participant = await getParticipantForSession(client, user, sessionId);
...
export async function createMarketOffer(user: AuthenticatedUser, rawPayload: unknown) {
  ...
  try {
    ...
    const { participant, session, currentMinute } = await getParticipantAndLockedSession(
      client,
      user,
      payload.sessionId
    );
...
export async function acceptMarketOffer(user: AuthenticatedUser, rawPayload: unknown) {
  ...
  try {
    ...
    const {
      ...
      session,
      currentMinute,
    } = await getParticipantAndLockedSession(client, user, payload.sessionId);
    ...
    const averagePriceBeforeTrade = Number(economyBeforeTradeResult.rows[0]?.average_price ?? 1);

    const overpricePressure = calculateOverpricePressure(pricePerUnit, averagePriceBeforeTrade);

    const underpricePressure = calculateUnderpricePressure(pricePerUnit, averagePriceBeforeTrade);
    ...
    const sellerParticipantId = offerType === 'sell' ? creatorParticipantId : acceptorParticipantId;

    const buyerParticipantId = offerType === 'sell' ? acceptorParticipantId : creatorParticipantId;
    ...
    const demandSupplyPressure = await calculateDemandSupplyPressure(client, payload.sessionId, {
    ...
    await applyEconomyPressuresAndSaveSnapshot(client, payload.sessionId, 'trade', {
