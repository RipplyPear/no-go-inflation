...
import {
  ...
  joinLobby,
  startLobbySession,
} from '../game/services/session.service';
  ...
  switch (message.type) {
    ...
    case 'LEAVE_LOBBY': {
      ...
      try {
        ...
        if (result.cancelled) {
          broadcastSessionCancelled(result.sessionId, result.reason ?? 'Lobby-ul a fost închis.');
        } else if (result.shouldBroadcastLobby) {
          await broadcastLobbyStateToSession(result.sessionId);
        }
      } catch (error) {
        sendJson(ws, 'ERROR', {
          message: error instanceof Error ? error.message : 'Nu s-a putut părăsi lobby-ul.',
        });
      }
    ...
    case 'LEAVE_SESSION': {
      ...
      try {
        ...
        if (result.cancelled) {
          broadcastSessionCancelled(result.sessionId, result.reason ?? 'Sesiunea a fost oprită.');
        } else {
        ...
        sendJson(ws, 'ERROR', {
          message: error instanceof Error ? error.message : 'Nu s-a putut părăsi sesiunea.',
        });
      }
    ...
    case 'CREATE_DEMO_SESSION': {
      ...
      try {
        ...
        sendJson(ws, 'ERROR', {
          message: error instanceof Error ? error.message : 'Could not create demo session.',
        });
      }
    ...
    case 'RECYCLE_RESOURCE': {
      ...
      try {
        ...
        sendJson(ws, 'ERROR', {
          message: error instanceof Error ? error.message : 'Reciclarea a eșuat.',
        });
      }
    ...
    case 'GET_MARKET_STATE': {
      ...
      try {
        ...
        const marketState = await getMarketStateForUser(user, payload.sessionId);

        sendJson(ws, 'MARKET_STATE', marketState);
      } catch (error) {
        sendJson(ws, 'ERROR', {
          message: error instanceof Error ? error.message : 'Nu s-a putut încărca piața.',
        });
      }
    ...
    case 'CREATE_MARKET_OFFER': {
      ...
      try {
        ...
        sendJson(ws, 'ERROR', {
          message: error instanceof Error ? error.message : 'Crearea ofertei a eșuat.',
        });
      }
    ...
    case 'ACCEPT_MARKET_OFFER': {
      ...
      try {
        ...
        sendJson(ws, 'ERROR', {
          ...
          offerId,
          message: error instanceof Error ? error.message : 'Acceptarea ofertei a eșuat.',
        });
      }
    ...
    case 'CANCEL_MARKET_OFFER': {
      ...
      try {
        ...
        sendJson(ws, 'ERROR', {
          message: error instanceof Error ? error.message : 'Nu s-a putut retrage oferta.',
        });
      }
    ...
    case 'DEV_SEED_BOT_OFFER': {
      ...
      try {
        ...
        sendJson(ws, 'ERROR', {
          message: error instanceof Error ? error.message : 'Nu s-a putut crea oferta de test.',
        });
      }
    ...
    case 'DEV_FORCE_FINISH_SESSION': {
      ...
      try {
        const finalResult = await forceFinishSessionForTesting(user, message.payload);
        ...
        sendJson(ws, 'ERROR', {
          message: error instanceof Error ? error.message : 'Nu s-a putut finaliza sesiunea.',
        });
      }
